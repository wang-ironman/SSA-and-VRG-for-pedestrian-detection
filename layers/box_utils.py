...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    """
    """
    overlaps Shape为[truths.shape[0],priors.shape[0]],每一行对应truths中的一个框
    和priors中所有框的iou
    """
    # jaccard index
...
def match_vis(threshold, truths, priors, variances, labels, loc_t,conf_t, idx):
    """Match each prior box with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    Args:
    ...
    for j in range(best_prior_idx.size(0)):
        best_truth_idx[best_prior_idx[j]] = j
    matches = truths[best_truth_idx]          # Shape: [num_priors,4]记录的是每个默认框对应的真实值的归一化的回归值
    # print(matches.cpu().numpy())
    conf = labels[best_truth_idx] + 1         # Shape: [num_priors]
    conf[best_truth_overlap < threshold] = 0  # label as background
    loc = encode(matches, priors, variances)
...
def nms_v(boxes, scores, overlap=0.55, top_k=300):
