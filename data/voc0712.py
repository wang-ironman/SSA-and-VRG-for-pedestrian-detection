...
class VOCDetection(data.Dataset):
    ...
    def __getitem__(self, index):
        im, gt, gt_vis, h, w = self.pull_item(index)
        # print("im type")
        # print(type(im))
        return im, gt, gt_vis
    ...
    def pull_item(self, index):
        ...
        if self.transform is not None:
            ...
            img, boxes, labels, boxes_vis, labels_vis = self.transform(img, target[:, :4], target[:, 4],target_vis[:, :4], target_vis[:, 4])
            # print('test1')
            # print(target.shape)
            # print(img.shape)
            # print(boxes.shape)
        ...
        # .astype(np.float32) 添加np.float32, 图片使用浮点型
        return torch.from_numpy(img.astype(np.float32)).permute(2, 0, 1), target, target_vis, height, width
    ...
    def pull_anno_vis(self, index):
        ...
        gt_vis = self.target_transform(anno_vis, 1, 1)
...
def detection_collate(batch):
    ...
    return torch.stack(imgs, 0), targets, targets_vis
